...
+----------+----------------+
           |
           | POST /analyze?url=...        POST /rule/classify?url=...
           v                                        |
+---------------------------+                       |
| FastAPI Worker API        |<----------------------+
|---------------------------|
...
+----------+----------------+
           |                \
           | enqueue         \ (rule: API 프로세스 내 동기 실행)
           v                  v
+---------------------------+ +---------------------------+
| Redis Queue (Celery)      | | RuleAnalyzer              |
|---------------------------|  |---------------------------|
| - 작업 큐                  | | - 8단계 파이프라인        |
| - Job 상태 저장소          | | - 키워드 매칭             |
+----------+----------------+ | - anti-bot 완충           |
...
RuleAnalyzer.analyze_website(url)
  ...
  ├─ 4. DB 저장: anti_bot_blocked=True 이면 ai_site.status = "blocked"
  └─ 5. 즉시 결과 반환
```
