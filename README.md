...
| **Phase 9** | 신뢰도 기반 캐시 스킵 및 DB 스키마 개선 | ✅ 완료 |
...
DB에 이미 분석 결과가 있으면 파이프라인을 실행하지 않고 캐시를 반환한다:
- LLM 분석 결과(`analyzer != "rule"`)가 있는 경우 → 즉시 반환
- 규칙기반 결과이고 `hard_pass=true`, `review_required=false`, `total_score >= 60.0`을 모두 충족하는 경우 → 즉시 반환
...
- **특징**:
  ...
  - **신뢰도 기반 캐시 스킵**: DB에 충분히 신뢰할 수 있는 결과가 있으면 재분석 생략
    - LLM 결과(`analyzer != "rule"`) → 항상 캐시 반환
    - 규칙기반 결과 + `hard_pass=true` + `review_required=false` + `total_score >= 60.0` → 캐시 반환
...
**마지막 업데이트**: 2026-05-15 · **상태**: ✅ Phase 9 완료, 배포 준비 완료
