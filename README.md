...
| **캐시/큐** | Redis | 7+ |
| **비동기 작업** | Celery | 5.6+ |
| **AI API** | Gemini (기본) / Claude (선택) | gemini-3.1-flash-lite |
| **테스팅** | pytest | 7.0+ |
| **패키지 관리** | uv | - |
...
- `LLM_PROVIDER=gemini` (기본): Gemini API로 웹사이트 분석
  ...
  - Free tier 활용 가능
- `LLM_PROVIDER=claude` **(미구현)**: Claude 분석기가 아직 구현되지 않았습니다. 설정해도 Gemini로 폴백됩니다.

## 분류기 모드 선택

- `CLASSIFIER_MODE=llm` (기본): LLM API로 웹사이트 분석 (현재: Gemini)
